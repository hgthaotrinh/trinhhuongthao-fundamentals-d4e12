data = {                                                    # nên lướt qua kiểu dữ liệu để xem có lỗi gì không
    "total_results": 3,
    "results": [
        {
            "popularity": 512.119,
...
for movie in movie_list:
    if movie['vote_count'] > 500:                           # in phim có > 500 votes
        print(movie['title'], movie['vote_count'])
print()


gen_input = int(input('Nhập số genre bạn thích: '))
for movie in movie_list:
    for gen in movie['genres']:                              # in phim có genre bằng bao nhiêu
        if gen == gen_input:
            print(movie['title'], 'genres', gen)
print()


for movie in movie_list:
    print(movie['title'], movie['release_date'])
    year = movie['release_date']                             # in phim release năm 2019
    year = year.split('-')[0]                                # vị trí số 0
    for i in year:
        if int(i) == 2019:
            print(movie['title'], 'released in 2019')

# for movie in movie_list:
#     rl_date = movie['released_date']
#     if '2019' in rl_date:         #cách 2